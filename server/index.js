...
async function start() {
  ...
  if (GEMINI_API_KEY) {
    ...
    geminiInitPromise = initGeminiFiles(genaiClient)
      ...
      .catch((e) =>
        console.warn(
          "[gemini-files] 初期化失敗（フォールバック継続）:",
          e.message,
        ),
      );
  } else {
    console.warn(
      "[advice] GEMINI_API_KEY 未設定: LLMアドバイスは無効（ルールベースフォールバックのみ）",
    );
  ...
  try {
    ...
    if (total === 0)
      return res.json({
        ...
        sameDestinationRate: 0,
        batteryRentalRate: 0,
        cashWithdrawRate: 0,
        noSnsRate: 0,
        sameDestinationFacilityName: null,
      });
    ...
    const finalDestinationDist = Object.entries(areaCounts).map(
      ([area, count]) => ({
        area,
        count,
    ...
    const sameCount = finalDestination
      ? allResults.filter(
          (r) => getFinalFac(r.EventHistory) === finalDestination,
        ).length
      : 0;
    const batteryCount = allResults.filter(
      (r) =>
        Array.isArray(r.EventHistory) &&
        r.EventHistory.some((e) => e.id === "event_walk_001"),
    ).length;
    const cashCount = allResults.filter(
      (r) =>
        Array.isArray(r.EventHistory) &&
        r.EventHistory.some((e) => e.id === "event_walk_003"),
    ).length;
    const noSnsCount = allResults.filter(
      (r) =>
        !Array.isArray(r.EventHistory) ||
        !r.EventHistory.some((e) => e.id?.startsWith("event_sns_")),
    ).length;
