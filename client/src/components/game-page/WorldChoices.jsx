...
const normalizeDist = (dist) => {
  ...
  return AREA_ORDER.filter((area) => merged[area] > 0).map((area) => ({
    area,
    count: merged[area],
...
export const WorldChoices = ({ finalDestination }) => {
  ...
  useEffect(() => {
    const query = finalDestination
      ? `?finalDestination=${finalDestination}`
      : "";
    fetch(`${BASE_URL}/api/results/stats${query}`)
      .then((r) => {
  ...
  // エリアデータを正規化・ソート
  const normalizedDist = stats?.finalDestinationDist
    ? normalizeDist(stats.finalDestinationDist)
    : [];
  ...
  return (
    <Flex className="world-choices" width="90%" flexDirection="column" mt="2vh">
      {/* ヘッダー */}
      <Flex
        paddingY="1vh"
        paddingX="4%"
      ...
      >
        ...
        {!loading && !error && stats?.total > 0 && (
          <>
            ...
            <Text className="text-subtext">全 {stats.total} 人のデータ</Text>
                ...
                <Flex key={item.area} alignItems="center" gap="0.5vw">
                  <div
                    style={{
                      ...
                      border:
                        item.area === "その他" ? "0.1vh solid #aaa" : "none",
                      flexShrink: 0,
                    }}
                  />
            ...
            <Flex flexWrap="wrap" width="100%" gap="1.5vh" mt="0.5vh">
              ...
              <DonutChart
                rate={stats.batteryRentalRate}
                label="充電スポットに立ち寄った"
              />
              <DonutChart
                rate={stats.cashWithdrawRate}
                label="現金を確保した"
              />
