...
1. **主経路（LLM）**: サーバーの `GET /api/advice` が Gemini API（`gemini-2.5-flash`）で防災ナレッジPDFを参照し、ユーザーの行動履歴をもとに **100字程度のアドバイス1件** を生成する。
...
- **モデル**: `gemini-2.5-flash`, `temperature=0.2`, `thinking_budget=0`
- **システムプロンプト**: 渋谷での地震避難体験者へのフィードバック。防災資料のみ根拠とし、改行・箇条書きなしで100字前後のワンセンテンスで返す。
